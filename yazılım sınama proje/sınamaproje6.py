...
def list_data():
    ...
    list_window.title("Veri Listesi")
...
app.configure(bg="#e0f7fa")  # Arka plan rengini açık turkuaz yap
app.eval('tk::PlaceWindow %s center' % app.winfo_toplevel())  # Pencereyi ekranın ortasına yerleştir

# Etiketler ve giriş alanları
name_label = tk.Label(app, text="Sahibin Adı:", bg="#e0f7fa", font=("Arial", 14))
name_label.pack(pady=10)
name_entry = tk.Entry(app, width=50, font=("Arial", 14))
name_entry.pack(pady=10)

file_label = tk.Label(app, text="Ses Dosyası:", bg="#e0f7fa", font=("Arial", 14))
file_label.pack(pady=10)
file_frame = tk.Frame(app, bg="#e0f7fa")
file_frame.pack(pady=10)
file_entry = tk.Entry(file_frame, width=40, font=("Arial", 14))
file_entry.pack(side=tk.LEFT, padx=5)
file_button = tk.Button(file_frame, text="Seç", command=select_file, font=("Arial", 14))
file_button.pack(side=tk.LEFT)

# Butonları hizalamak için Frame kullanımı
frame = tk.Frame(app, bg="#e0f7fa")
frame.pack(padx=20, pady=20)

# Sol alttaki butonları hizalama
left_frame = tk.Frame(frame, bg="#e0f7fa")
left_frame.grid(row=0, column=0, padx=10, pady=10, sticky="w")

# Sağ alttaki butonları hizalama
right_frame = tk.Frame(frame, bg="#e0f7fa")
right_frame.grid(row=0, column=1, padx=10, pady=10, sticky="e")

# Sol alttaki butonlar
save_button = tk.Button(left_frame, text="Kaydet", command=save_data, bg="#ff7070", width=20, font=("Arial", 14))
save_button.pack(pady=5)

record_button = tk.Button(left_frame, text="Kayda Başla", command=start_recording, bg="#f3e5f5", width=20, font=("Arial", 14))
record_button.pack(pady=5)

stop_button = tk.Button(left_frame, text="Kaydı Durdur", command=stop_recording, bg="#f3e5f5", width=20, font=("Arial", 14))
stop_button.pack(pady=5)

# Sağ alttaki butonlar
list_button = tk.Button(right_frame, text="Listele", command=list_data, bg="#f3e5f5", width=20, font=("Arial", 14))
list_button.pack(pady=5)

# Buton yazısını kısaltarak butona sığmasını sağlıyoruz
plot_button = tk.Button(right_frame, text="Dalga ve Spektrogram", command=display_waveform_and_spectrogram, bg="#f3e5f5", width=20, font=("Arial", 14))
plot_button.pack(pady=5)

train_button = tk.Button(right_frame, text="Model Eğit ve Değerlendir", command=train_and_evaluate_model, bg="#f3e5f5", width=20, font=("Arial", 14))
